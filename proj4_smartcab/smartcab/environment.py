    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                self.done = True
                print "Environment.act(): Primary agent has reached destination!!!!!!!! with DEADLINE ==== ", state['deadline'], "Net_Reward = ", self.trial_data['net_reward']   # [debug]
                self.track_deadline.append(state['deadline'])
                self.track_net_reward.append(self.trial_data['net_reward'])
            self.status_text = "state: {}\naction: {}\nreward: {}".format(agent.get_state(), action, reward)
            #print "Environment.act() [POST]: location: {}, heading: {}, action: {}, reward: {}".format(location, heading, action, reward)  # [debug]

            ### oflucas adding
            #if state['deadline'] < 0: reward -= 0.5
